...
const { aboutMenu, grupMenu, funMenu, downloadMenu, mediaMenu, audioMenu, makerMenu, ownerMenu, donasiBot, bahasaBot } = require('./Faa/pinkyca.js')
const { rules, limitend, limitcount } = require('./Faa/help')
	...
	nuy.on('chat-update', async (caa) => {
		try {
         ...
         const listmsg = (from, title, desc, list) => {
            let faa = nuy.prepareMessageFromContent(from, {"listMessage": {"title": title, "description": desc, "buttonText": "List Menu", "footerText": "Base: FxSx & FdlX", "listType": "SINGLE_SELECT", "sections": list}}, {quoted: caa})
            return nuy.relayWAMessage(faa, {waitForAck: true})
         }
         const listmsgc = (from, title, desc, list) => {
            let faa = nuy.prepareMessageFromContent(from, {"listMessage": {"title": title, "description": desc, "buttonText": "List Info", "footerText": "Base: FxSx & FdlX", "listType": "SINGLE_SELECT", "sections": list}}, {quoted: caa})
            return nuy.relayWAMessage(faa, {waitForAck: true})
         }
         ...
         mess = {
       	...
       	if (!packname) packname = 'FxBot'; if (!author) author = 'BaseFxSxFdlX';	
        ...
        await nuy.sendMessage(from, `${nuyfaa}`, text, options, {quoted: ftroli, contextInfo: {forwardingScore: 99999, isForwarded: true}})
          ...
          case 'help':
               ...
               list = []
               listmenu = [`aboutmenu`,`info`,`grupmenu`,`funmenu`,`downloadmenu`,`mediamenu`,`audiomenu`,`makermenu`,`ownermenu`]
               listmenuu = [`Menu About`,`Info Bot`,`Menu Grup`,`Menu Fun`,`Menu Download`,`Menu Media`,`Menu Audio`,`Menu Maker`,`Menu Owner`]
               urutan = 1
               startnum = 0
               for (let x of listmenu) {
               const fxsx = {title: 'Salahkan Pilih >' + urutan++,
                    rows: [
                       {
          ...
          case 'here':                
           ...
           case 'bahasa':
				   if (isBanned) return fakeitem(mess.only.benned)    
				   if (!isUser) return fakeitem(mess.only.userB)
			   	nuy.sendMessage(from, bahasaBot(prefix), MessageType.text, {quoted: ftroli})
			   	break
           case 'pale':
            ...
            case 'fotomem':
		         if (!isUser) return fakeitem(mess.only.userB)
				   ...
				   if (caa.message.extendedTextMessage != undefined){
					...
					nuy.sendMessage(from, thumb, MessageType.image, {quoted: ftroli})
				   break
