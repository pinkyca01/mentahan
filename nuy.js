	...
	nuy.on('chat-update', async (caa) => {
		try {
         ...
         responseButton = (type == 'listResponseMessage') ? caa.message.listResponseMessage.title : ''
            ...
            let po = nuy.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "List Pinky","footerText": "❣ Base : FxSx FdlX\n❣ Recode : Pinky\n❣ Note : Mohon Jangan Spam Bot Pinky!!","listType": "SINGLE_SELECT","sections": list}}, {})
            return nuy.relayWAMessage(po, {waitForAck: true})
         }
			const isUrl = (url) => {
			    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
			}
			const reply = (teks) => {
				nuy.sendMessage(from, teks, text, {quoted: { key: { fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? {remoteJid :"6283815956151-1604595598@g.us" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('pinky.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": name,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, quotedMessage: {"extendedTextMessage": {"matchedText": "https://wa.me/message/4CFWJARLRISPA1", "description": "Akun Bisnis", "title": "PinkyCa", "previewType": true, "jpegThumbnail": fs.readFileSync('pinky.jpg')}}, contextInfo: {"forwardingScore":999999,"isForwarded":true},sendEphemeral: true})
			}
			const fakeitem = (teks) => {
            nuy.sendMessage(from, teks, text, {quoted: { key: { fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? {remoteJid :"6283815956151-1604595598@g.us" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('pinky.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": name,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999999,"isForwarded":true},sendEphemeral: true})
         }
        ...
        await nuy.sendMessage(from, `${nuyfaa}`, text, options, {quoted: ftroli, contextInfo: {forwardingScore: 99999, isForwarded: true}})
        }
          ...
          case 'help':
               ...
               menu = `Hai ${pushname}
               Pinky Bot Disini Senang Bertemu Dengan Anda!!
               Silahkan Pilih Salah Satu Di Bawah`
               buttons = [{buttonId: `${prefix}cmd`,buttonText:{displayText: 'LIST PINKY'},type:1},{buttonId:`${prefix}donasi`,buttonText:{displayText:'DONASI'},type:1},{buttonId:`${prefix}info`,buttonText:{displayText:'INFO PINKY'},type:1}]
               imageMsg = (await nuy.prepareMessageMedia(fs.readFileSync('pinky.jpg'), 'imageMessage', {thumbnail: fs.readFileSync('pinky.jpg')})).imageMessage
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '❣ Base : FxSx FdlX\n❣ Recode : Pinky\n❣ Note : Mohon Jangan Spam Bot Pinky!!', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
               }
          ...
          case 'cmd':
               ...
               if (!isUser) return fakeitem(mess.only.userB)
               list = []
               listmenu = [`grupmenu`,`aboutmenu`,`funmenu`,`downloadmenu`,`mediamenu`,`audiomenu`,`makermenu`,`ownermenu`]
               listmenuu = [`Menu Grup`,`Menu About`,`Menu Fun`,`Menu Download`,`Menu Media`,`Menu Audio`,`Menu Maker`,`Menu Owner`]
               urutan = 1
               startnum = 0
               ...
               listmsg(from, `${ucapanWaktu}`, `Hai ${pushname}, Menu Pinky Disini`, list)
               break
          case 'grupmenu': 
...
case 'trigger':
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
          ...
          case 'profile':
				...
				if (isBanned) return fakeitem(mess.only.benned)    
				if (!isUser) return fakeitem(mess.only.userB)
				nuy.sendMessage(from, bahasa(prefix), text, {quoted: ftroli})
				break
				case 'info':
		   		if (isBanned) return fakeitem(mess.only.benned)    
			   	if (!isUser) return fakeitem(mess.only.userB)
               ...
               infobot = `${fx} Nama Bot : ${me.name}
               ${fx} Owner Bot : Privat!
               ${fx} Prefix : | ${prefix} |
               ${fx} Total Block : ${blocked.length}
               ${fx} Aktif Sejak : ${kyun(uptime)}
               ${fx} Total User : ${user.length}`
               buttons = [{buttonId: `${prefix}totaluser`,buttonText:{displayText: 'USER LIST'},type:1},{buttonId:`${prefix}blocklist`,buttonText:{displayText:'BLOCK LIST'},type:1},{buttonId: `${prefix}banlist`,buttonText:{displayText: 'BAN LIST'},type:1}]
               buttonsMessage = {
               contentText: `${infobot}`,
               footerText: '❣ Base : FxSx FdlX\n❣ Recode : Pinky\n❣ Note : Mohon Jangan Spam Bot Pinky!!',
               buttons: buttons,
               headerType: 1
               }
               prep = await nuy.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftroli, contextInfo: { forwardingScore: 99999, isForwarded: true}})
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           case 'storyanime':
               if (isBanned) return fakeitem(mess.only.benned)    
					if (!isUser) return fakeitem(mess.only.userB)
					if (isLimit(sender)) return fakeitem(limitend(pushname))
               if (!isAnime) return fakeitem(`Silahkan Ketik ${prefix}anime on [Khusus Admin]`)
               anu = await fetchJson(`https://lolhuman.herokuapp.com/api/storynime?apikey=${LolApi}`)
...
//setting bot
            ...
            case 'stikcmd':
               if (!isOwner) return fakeitem(mess.only.ownerB)
               if (isQuotedSticker) {
               if (!q) return fakeitem(`Penggunaan : ${prefix}${command} cmdnya dan tag stickernya`)
